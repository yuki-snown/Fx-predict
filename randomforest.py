...
rforest = RandomForestRegressor(n_estimators=50, random_state=1, n_jobs=-1)
rforest.fit(train_x1, train_y1)
pred_y1 = rforest.predict(test_x1)
pred_y1 = pred_y1.round(2)

check_x, check_y = make_data1(jpy[test_period2])
check_y1 = [0]
for i in range(day):
    check_y1.append(check_y[i+60])
del check_y1[0]

train_x2, train_y2 = make_data1(jpy[train_period])
test_x2, test_y2 = make_data1(jpy[test_period1])
rforest = RandomForestRegressor(n_estimators=50, random_state=1, n_jobs=-1)
rforest.fit(train_x2, train_y2)
test_y2.append(np.nan)
for i in range(day):
    pred_y2 = rforest.predict(test_x2)
